  ...
  /* Handle other commands only if connected */
    ...
    switch (cmd) {
      ...
      case CC_UPLOAD: /* Upload */
        {
          #define size com[2]
          ccpReadMTA(0,size,&ccp.Crm[3]);  //注意此处是Crm[3],size最大可取5
          #ifdef CCP_TESTMODE
            if (gDebugLevel) {
              int i;
      ...
      case CC_GET_CCP_VERSION: /* Get Version */
        ...
        break;
      case CC_GET_CALRAM_MTA: //获取标定RAM的地址(全局地址)
        {
          *(CCP_DWORD*)&com[4] = (CCP_DWORD)((CCP_MTABYTEPTR)&ccpCalBuffer[0]);
        }
        break;
      case CC_GET_PROGRAM_MTA: //获取程序刷新RAM的地址
        {
          *(CCP_DWORD*)&com[4] = (CCP_DWORD)((CCP_MTABYTEPTR)&ccpPROGRAMBuffer[0]);  
        }
        break;
