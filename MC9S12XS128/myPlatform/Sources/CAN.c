...
{
    word ccpID;
    //word firstID,secondID;
    
    ccpID    = CCP_CRO_ID;   //ccp标定用命令发送对象ID 
    //firstID  = CAN0_RX_First_ID;
    //secondID = CAN0_RX_Second_ID;  
    ...
    //对相应ID进行滤波
    //第一帧
    CAN0IDAR0 = (byte)(ccpID>>3);
    CAN0IDAR1 = (byte)(ccpID<<5);
    CAN0IDAR2 = 0;
    CAN0IDAR3 = 0;
    //第二帧
    CAN0IDAR4 = 0;
...
void BusOff_Update(void)
{
  ...
  {
    canBusOffTimeCnt++;
    if( canBusOffTimeCnt >= 50) //延时50*bus-off period
...
void canMessageWrite(void)
{
    ...
    msg_send.data[0] = adValue%256;
    msg_send.data[1] = adValue/256;
    msg_send.data[2] = speed%256;
    msg_send.data[3] = speed/256; 
