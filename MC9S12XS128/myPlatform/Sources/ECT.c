...
word readSpeed(void) 
{
  ...
  if(Pca.IC0_Buffer[IC0_BUFFERSIZE-1] != 0)
  {
    for(i=0;i<3;i++)
    {
      Max = Pca.IC0_Buffer[5*i];    //必须在之前将MAX于MIN初始化，否则会遗传上一次的计数值，导致溢出
      Min = Pca.IC0_Buffer[5*i];
      for(j=0;j<5;j++)
      {
        Sum[i] += Pca.IC0_Buffer[5*i+j];
        if(Max <= Pca.IC0_Buffer[5*i+j])
          Max = Pca.IC0_Buffer[5*i+j];
        else if(Min >= Pca.IC0_Buffer[5*i+j])
          Min = Pca.IC0_Buffer[5*i+j];
      } 
      Sum2 += (Sum[i]-Max-Min)/3;
      Sum[i] = 0;  
    }
    aveValue = Sum2/3;
